...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    if (verifyCsrf($_POST['csrf'] ?? '')) {
        $id = (int)($_POST['id'] ?? 0);
        if ($id > 0) { Database::query('DELETE FROM ramais WHERE id = ?', [$id]); $msg = 'Ramal excluído.'; }
    }
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    if (!verifyCsrf($_POST['csrf'] ?? '')) { $msg = 'Token inválido.'; $msgType = 'danger'; } else {
        $id = (int)($_POST['id'] ?? 0); $andar = trim($_POST['andar'] ?? ''); $setor = trim($_POST['setor'] ?? ''); $ramal = trim($_POST['ramal'] ?? ''); $linha = trim($_POST['linha'] ?? '');
        if (!$andar || !$setor || !$ramal) { $msg = 'Preencha Andar, Setor e Ramal.'; $msgType = 'danger'; }
        elseif ($id > 0) { Database::query('UPDATE ramais SET andar=?, setor=?, ramal=?, linha=?, updated_at=NOW() WHERE id=?', [$andar, $setor, $ramal, $linha, $id]); $msg = 'Ramal atualizado.'; }
        else { $maxOrder = Database::fetch('SELECT MAX(sort_order) as m FROM ramais')['m'] ?? 0; Database::query('INSERT INTO ramais (andar, setor, ramal, linha, sort_order) VALUES (?,?,?,?,?)', [$andar, $setor, $ramal, $linha, $maxOrder + 1]); $msg = 'Ramal adicionado.'; }
    }
}

$busca = trim($_GET['q'] ?? ''); $andarF = trim($_GET['andar'] ?? ''); $where = 'WHERE 1=1'; $params = [];
  ...
  <main class="admin-main">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:28px;flex-wrap:wrap;gap:12px">
      <div>
        ...
        <p class="text-muted text-sm"><?= count($ramais) ?> ramal(is) cadastrado(s)</p>
      </div>
      <button onclick="document.getElementById('modalNovo').style.display='flex'" class="btn btn-primary">
        <span class="material-icons">add</span> Novo Ramal
      </button>
    </div>

    <?php if ($msg): ?>
    <div class="alert alert-<?= $msgType ?>" style="margin-bottom:20px">
      <span class="material-icons"><?= $msgType==='success'?'check_circle':'error' ?></span>
    ...
    <div class="card" style="overflow:hidden;padding:0">
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse">
          ...
          <tr style="border-bottom:1px solid var(--border)" onmouseover="this.style.background='var(--primary-xlight)'" onmouseout="this.style.background=''">
            ...
            <td style="padding:11px 16px"><span style="background:#f0fdf4;border:1px solid #bbf7d0;color:#15803d;padding:3px 10px;border-radius:20px;font-size:13px;font-weight:700"><?= htmlspecialchars($r['ramal']) ?></span></td>
            <td style="padding:11px 16px;font-size:13px;color:var(--text-muted)"><?= $r['linha'] ? htmlspecialchars($r['linha']) : '—' ?></td>
          ...
          <?php endforeach; ?>
          <?php if (empty($ramais)): ?>
          <tr><td colspan="5" style="padding:40px;text-align:center;color:var(--text-muted)">Nenhum ramal encontrado.</td></tr>
          <?php endif; ?>
          </tbody>
        </table>
      </div>
...
<div id="modalNovo" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:1000;align-items:center;justify-content:center">
  <div class="card" style="width:100%;max-width:480px;padding:28px;margin:20px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
      ...
      <button onclick="document.getElementById('modalNovo').style.display='none'" style="background:none;border:none;cursor:pointer;color:var(--text-muted)"><span class="material-icons">close</span></button>
    </div>
    <form method="POST">
      <input type="hidden" name="csrf" value="<?= csrf() ?>">
      ...
      <div class="form-group"><label class="form-label">Linha Direta</label><input type="text" name="linha" class="form-control" placeholder="Ex: (91) 3197-6328"></div>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:20px">
        <button type="button" onclick="document.getElementById('modalNovo').style.display='none'" class="btn btn-ghost">Cancelar</button>
...
<!-- Modal Editar -->
<?php if ($editRamal): ?>
<div style="display:flex;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:1000;align-items:center;justify-content:center">
  <div class="card" style="width:100%;max-width:480px;padding:28px;margin:20px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    ...
    <form method="POST">
      ...
      <div class="form-group"><label class="form-label">Linha Direta</label><input type="text" name="linha" class="form-control" value="<?= htmlspecialchars($editRamal['linha']) ?>"></div>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:20px">
